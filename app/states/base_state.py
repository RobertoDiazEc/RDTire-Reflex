...
class BaseState(rx.State):
    ...
    def unique_brands(self) -> list[str]:
        return []
    #//sorted(list(set((t["brand"] for t in self.tires))))//

    @rx.var
    def unique_sizes(self) -> list[str]:
        return []
    #*/sorted(list(set((t["size"] for t in self.tires))))/*

    @rx.var
    def unique_types(self) -> list[str]:
        return [] 
    #sorted(list(set((t["type"] for t in self.tires))))

    @rx.var
    def unique_seasons(self) -> list[str]:
        return []
    #sorted(list(set((t["season"] for t in self.tires))))