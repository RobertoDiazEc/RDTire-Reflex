...
def products_edit_lateral(tire: Tire) -> rx.Component:
    return rx.el.div(
        rx.el.button(
                    rx.icon("circle-fading-plus", class_name="h-4 w-4"),
                    on_click=lambda: TiresState.open_product_detail_modal(tire),
                    class_name="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-80 rounded-md",
                ),
        rx.cond(
            TiresState.show_product_detail_modal,
...
def products_page_ui() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.div(
            ...
            rx.el.button(
                "Nuevo",
                on_click=TiresState.open_add_product_modal(AuthState.current_user["cliente_id"]),
            ...
            rx.button(
                ...
                on_click=TiresState.mostrar_tires(AuthState.current_user["cliente_id"]),
            ),
            class_name="mb-6",
        ),
        rx.el.div(
            # BaseState.filtered_tires
            rx.foreach(TiresState.tires, product_card),
        ...
        class_name="p-4 max-h-[220px] w-full ",
    )


def product_detail_modal() -> rx.Component:
    return rx.cond(
        TiresState.show_product_detail_modal & TiresState.selected_tire,
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    rx.el.h2(
                        f"{TiresState.selected_tire['brand']} {TiresState.selected_tire['size']}",
                        class_name="text-2xl font-bold",
                    ),
                    rx.el.button(
                        rx.icon("x", class_name="h-5 w-5"),
                        on_click=TiresState.close_product_detail_modal,
                ...
                rx.el.div(
                    rx.image(
                        src=TiresState.selected_tire["image_url"],
                        class_name="w-full h-64 object-cover rounded-lg",
                    ),
                    rx.el.div(
                        rx.el.div(
                            rx.el.p("DOT:", class_name="font-semibold"),
                            rx.el.p(TiresState.selected_tire["dot"]),
                        ),
                        rx.el.div(
                            rx.el.p("Estado:", class_name="font-semibold"),
                            rx.el.p(TiresState.selected_tire["estado"]),
                        ),
                        rx.el.div(
                            rx.el.p("Asignado a Vehiculo:", class_name="font-semibold"),
                            rx.el.p(TiresState.selected_tire["asignado_a_vehiculo"]),
                        ),
                        rx.el.div(
                            rx.el.p("Codigo Velocidad:", class_name="font-semibold"),
                            rx.el.p(TiresState.selected_tire["speed_rating"]),
                        ),
                        rx.el.div(
                            rx.el.p("Indice de Carga:", class_name="font-semibold"),
                            rx.el.p(TiresState.selected_tire["load_index"]),
                        ),
                ...
                rx.el.div(
                    rx.el.p(
                        f"${TiresState.selected_tire['price']:.2f}",
                        class_name="text-3xl font-bold",
                    ),
                    rx.el.p(
                        f"In Stock: {TiresState.selected_tire['stock']}",
...
def add_product_modal() -> rx.Component:
    def form_field(
        ...
        return rx.el.div(
            ...
            rx.el.input(
                ...
                name=name,
                type=type,
                on_change=lambda val: TiresState.handle_new_product_change(name, val),
    ...
    return rx.cond(
        TiresState.show_add_product_modal,
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    ...
                    rx.el.button(
                        ...
                        on_click=TiresState.close_add_product_modal,
                        class_name="rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none",
                    ),
                ...
                rx.el.form(
                    rx.el.div(
                        ...
                        form_field("Inventario Individual", "stock", "maximo 4 Neumaticos", type="number"),
                        form_field("Type", "type", "e.g. RADIAL"),
                    ...
                    on_submit=TiresState.save_new_product,
                ),
