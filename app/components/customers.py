...
def customer_list_item(customer: Customer) -> rx.Component:
    return rx.el.tr(
        rx.el.td(
            rx.el.div(
                rx.image(
        ...
        rx.el.td(
            rx.el.div(
                ...
                rx.el.button(
                    "Edit",
                    on_click=lambda: CustomerState.open_edit_customer_modal(customer),
...
def customer_modal() -> rx.Component:
    def form_field(
        label: str, name: str, value: str, placeholder: str, type: str = "text"
    ) -> rx.Component:
        return rx.el.div(
    ...
    return rx.cond(
        CustomerState.show_customer_modal,
        rx.el.div(
            rx.el.div(
                rx.el.div(
                    rx.el.h2(
                        rx.cond(
                            CustomerState.is_editing_customer,
                            "Edit Customer",
                            "Add New Customer",
                        ),
...
def customers_page_ui() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            ...
            rx.el.button(
                "Add Customer",
                on_click=CustomerState.open_add_customer_modal,
        ...
        rx.el.div(
            rx.el.table(
                ...
                rx.el.tbody(rx.foreach(CustomerState.customers, customer_list_item)),
            ),
